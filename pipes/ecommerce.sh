...
PROJECT_DIR="/opt/workspace/ecommerce"
...
if [ ! -d "$PROJECT_DIR" ]; then
    ...
    git clone https://${PROJECT_TOKEN}:x-oauth-basic@github.com/hugopaul/ecommerce.git "$PROJECT_DIR"
else
...
docker build -f Dockerfile -t hugopaul/ecommerce .

echo "######### Removendo o contêiner antigo, se existir ###########"
docker rm -f "ecommerce" || true

echo "######### Rodando o contêiner do Docker ###########"
docker run -d -p 5001:80 --name ecommerce --restart=always hugopaul/ecommerce
